...
export default class Faucet {
    ...
    async send(address: string) {
        ...
        let ad= "5HKYEmof8yaKoyCX9LauKFz3n6YLBEWGHii4BkrgvGymz35W"

        const check = checkAddress(ad, config.address_type);
        ...
        if (check[0]) {
            ...
            console.log(`Send ${config.amount} ${config.symbol} to ${ad}`);
            const tx = await this.api.get().tx.balances.transferKeepAlive(ad, amount).signAndSend(sender);
            console.log("Transfer sent with hash", tx.toHex());
            return `Sent ${config.amount} ${config.symbol} to ${ad} (tx ${tx.toHex()})`;
        }
