...
export default class Faucet {
  async send(address: string, amountToSend: number | undefined = undefined) {
    ...
    if (amountToSend === undefined) {
      amountToSend = config.faucet_amount;
    }
    ...
    if (check[0]) {
      ...
      const amount = new BN(amountToSend).mul(padding);
      console.log(`Send ${amountToSend} ${config.symbol} to ${address}`);
      ...
      return `Sent ${amountToSend} ${config.symbol} to ${address} (tx ${tx.toHex()})`;
    }
